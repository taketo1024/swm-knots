...
class SimpleModuleStructureTests: XCTestCase {
    ...
    func testFree() {
        ...
        let matrix = MatrixImpl<R>.zero(rows: 3, cols: 0)
    ...
    func testRelation() {
        typealias A = AbstractBasisElement
        let basis = (0 ..< 3).map{ A($0) }
        let matrix = MatrixImpl<R>(rows: 3, cols: 2, grid:[1, 0, 0, 2, 0, 0])
    ...
    func testSubsummands() {
        typealias A = AbstractBasisElement
        let basis = (0 ..< 3).map{ A($0) }
        let matrix = MatrixImpl<R>(rows: 3, cols: 2, grid:[2, 0, 0, 4, 0, 0])
        let str = S(generators: basis, relationMatrix: matrix)
