...
class MatrixDecompositionTests: XCTestCase {
    ...
    func testRowEliminationRandomM55() {
        for _ in 0 ..< 10 {
            let A = randomM55()
            let E = EuclideanMatrixElimination(A, mode: .RowsOnly)
    ...
    func testColEliminationRandomM55() {
        for _ in 0 ..< 10 {
            let A = randomM55()
            let E = EuclideanMatrixElimination(A, mode: .ColsOnly)
    ...
    private func randomSingularM55(shuffle s: Int = 10) -> M55 {
        ...
        for _ in 0 ..< s {
            ...
            switch rand(6) {
            case 0: A.addRow(at: i, to: j)
            case 1: A.multiplyRow(at: i, by: rand(1 ... 3))
            case 2: A.swapRows(i, j)
            case 3: A.addCol(at: i, to: j)
            case 4: A.multiplyCol(at: i, by: rand(1 ... 3))
            case 5: A.swapCols(i, j)
            default: ()
            }
        }
